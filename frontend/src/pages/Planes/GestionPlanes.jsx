...
import {
  ...
  FormControlLabel, Tooltip, useTheme, useMediaQuery,
} from '@mui/material';
...
export default function GestionPlanes() {
  ...
  const abrirEditar = (plan) => {
    setForm({ nombre: plan.nombre, tipo: plan.tipo, duracionDias: plan.duracionDias, precio: plan.precio, descripcion: plan.descripcion || '', activo: plan.activo });
      ...
      {isMobile ? (
        <Box display="flex" flexDirection="column" gap={1}>
          {loading ? (
            ...
            <Paper key={p.id} sx={{ p: 1.5, opacity: p.activo ? 1 : 0.55 }}>
              <Box display="flex" justifyContent="space-between" alignItems="center" mb={0.5}>
                <Typography fontWeight={700} fontSize={15}>{p.nombre}</Typography>
                <Typography fontWeight={700} color="success.main">${Number(p.precio).toLocaleString('es-AR')}</Typography>
              </Box>
              <Box display="flex" alignItems="center" gap={1} mb={p.descripcion ? 0.5 : 0}>
                <Chip label={p.tipo} size="small" variant="outlined" />
                <Typography variant="caption" color="text.secondary">{p.duracionDias} día{p.duracionDias !== 1 ? 's' : ''}</Typography>
              </Box>
              {p.descripcion && <Typography variant="caption" color="text.secondary" display="block" mb={0.5}>{p.descripcion}</Typography>}
            ...
            <TableBody>
              {loading ? (
                ...
                <TableRow key={p.id} hover sx={{ opacity: p.activo ? 1 : 0.5 }}>
                  ...
                  <TableCell sx={{ fontWeight: 700, color: 'success.main' }}>${Number(p.precio).toLocaleString('es-AR')}</TableCell>
      ...
      <Dialog open={open} onClose={() => setOpen(false)} maxWidth="sm" fullWidth fullScreen={isMobile}>
        ...
        <DialogContent>
          <Box display="flex" flexDirection="column" gap={2} mt={1}>
            ...
            <TextField label="Nombre del plan" name="nombre" value={form.nombre} onChange={handleChange} required fullWidth />
            ...
            <TextField label="Duración (días)" name="duracionDias" type="number" value={form.duracionDias}
              onChange={handleChange} required fullWidth inputProps={{ min: 1 }}
              helperText="Cantidad de días que dura el plan" />
            <TextField label="Precio ($)" name="precio" type="number" value={form.precio}
              onChange={handleChange} required fullWidth inputProps={{ min: 0, step: '0.01' }}
              helperText="Monto a cobrar por este plan" />
            <TextField label="Descripción (opcional)" name="descripcion" value={form.descripcion}
